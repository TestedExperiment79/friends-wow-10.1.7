...
function rematch.battle:PET_BATTLE_CLOSE()
    if settings.ShowAfterBattle and (self.wasInPVP or not settings.ShowAfterPVEOnly) and not rematch.frame:IsVisible() then
        rematch.frame:Show()
        rematch.frame:Configure(rematch.layout:GetLayout(C.STANDALONE))
    end
